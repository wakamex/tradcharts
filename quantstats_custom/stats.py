...
def momentum(returns, rf=0., rolling_period=126,
                   prepare_returns=True):
    ...
    res = ((1+returns).rolling(rolling_period).apply(_np.prod, raw=True) - 1) / \
        returns.rolling(100).std() / \
        _np.sqrt(rolling_period)

    return res

def momentum_simple(returns, rf=0., rolling_period=126,
                   prepare_returns=True):
